...
const AdminUsers = () => {
  ...
  const filteredUsers = useMemo(() => {
    ...
    return users.filter((user) => {
      ...
      const matchesRole =
        !filters.userRole || user.userRole === filters.userRole;
      const matchesStatus =
  ...
  const handleChangeStatus = async (
    user: UserSummary,
    nexStatus: ManagedUserStatus,
  ) => {
    if (user.accountStatus === nexStatus) return;

    try {
      setBusyUserId(user.userId);
      const updated = await updateUserAuthorization(user.userId, {
        accountStatus: nexStatus,
      });
      ...
      showSuccess("Estado de autorización actualizado correctamente.");
    } catch (error) {
      showError(
        error instanceof Error
          ? error.message
          : "No fue posible actualizar el estado de autorización.",
      );
    } finally {
      setBusyUserId(null);
    }
      ...
      <Stack spacing={3}>
        <Box>
          ...
          <Typography variant="body1" color="textSecondary">
            Supervisa los usuarios registrados y gestiona sus roles y
            autorizaciones desde este panel.
          </Typography>
        </Box>
